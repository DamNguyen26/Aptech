...
namespace EAP_C1809L_DamKhoiNguyen.Controllers
{
    ...
    public class AlbumsController : Controller
    {
        ...
        public ActionResult Index()
        {
        	...
        	{
        		case "Title":
        			albums = (Request["sort"]=="desc") ? albums.OrderByDescending(item => item.Title)
        			: albums.OrderBy(item => item.Title);
        			break;
        		default:
        			albums = albums.OrderBy(item => item.Title);
        			break;
        	}
        	...
        	if (!String.IsNullOrEmpty(Request["search"]))
        	{
        		...
        		albums = albums.Where(item =>item.Title.Contains(search));
        	}
